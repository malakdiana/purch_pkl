...
$chart1->setTopLeftPosition('D4');
$chart1->setBottomRightPosition('T20');
...
$objWorksheet->addChart($chart1);
   ...
   $excel->getActiveSheet()->getDefaultRowDimension()->setRowHeight(-1);
    ...
    $excel->getActiveSheet(0)->setTitle("Worksheet");
...
header('Content-Disposition: attachment;filename="Grafik amount supplier.xlsx"');
    ...
    $excel->getProperties()->setCreator('My Notes Code')
                 ->setLastModifiedBy('My Notes Code')
                 ->setTitle("Grafik amount section")
                 ->setSubject("Grafik")
                 ->setDescription("Laporan Grafik amount section")
                 ->setKeywords("Grafik amount section");
    // Buat sebuah variabel untuk menampung pengaturan style dari header tabel
    $style_col = array(
      'font' => array('bold' => true), // Set font nya jadi bold
      'alignment' => array(
    ...
    // Buat header tabel nya pada baris ke 3
    //$excel->setActiveSheetIndex(0)->setCellValue('A1', "no"); // Set kolom A3 dengan tulisan "NO"
    $excel->setActiveSheetIndex(0)->setCellValue('A1', "Section"); // Set kolom B3 dengan tulisan "NIS"
    $excel->setActiveSheetIndex(0)->setCellValue('B1', "Amount transaction "); // Set kolom C3 dengan tulisan "NAMA"
    // Set kolom E3 dengan tulisan "ALAMAT"
    ...
    // Panggil function view yang ada di SiswaModel untuk menampilkan semua data siswanya
    $bulandownload= $this->input->post('bulandownload2');
    $tahundownload= $this->input->post('tahundownload2');

    $tgldownload=$bulandownload."/".$tahundownload;
 //  echo $tgldownload;die();
    $siswa = $this->GrafikModel->getExport2($tgldownload);
    $no = 1; // Untuk penomoran tabel, di awal set dengan 1
    $numrow = 2; // Set baris pertama untuk isi tabel adalah baris ke 4
    foreach($siswa as $data){ // Lakukan looping pada variabel siswa
      //$excel->setActiveSheetIndex(0)->setCellValue('A'.$numrow, $data->no);
...
// Tentukan judul chart
$title1 = new PHPExcel_Chart_Title('Ammount Section');
...
header('Content-Disposition: attachment;filename="Grafik amount section.xlsx"');
    $write->save('php://output');
  }
